...
class CustomKeyMap<K extends readonly any[], V> {
...
export function createEmitters<T, TOptions extends object>(
  program: Program,
  TypeEmitterClass: typeof TypeEmitter<T, TOptions>,
  emitContext: EmitContext<TOptions>,
): [type: TypeEmitter<T, TOptions>, asset: AssetEmitter<T, TOptions>] {
  ...
  const typeToEmitEntity = new CustomKeyMap<[string, Type, ContextState], EmitEntity<T>>(([method, type, context]) => {
  ...
  const knownContexts = new CustomKeyMap<[LexicalTypeStackEntry, ContextState], ContextState>(([entry, context]) => {
  ...
  return [typeEmitter, assetEmitter];
  ...
  function setContextForType<TMethod extends TypeEmitterMethod>(
    ...
    for (const entry of lexicalTypeStack) {
      ...
      if (keyHasContext(entry.method)) {
        compilerAssert((typeEmitter as any)[lexicalKey], `TypeEmitter doesn't have a method named ${lexicalKey}`);
      }

      if (keyHasReferenceContext(entry.method)) {
        compilerAssert((typeEmitter as any)[referenceKey], `TypeEmitter doesn't have a method named ${referenceKey}`);
      }

      const newContext = keyHasContext(entry.method) ? (typeEmitter as any)[lexicalKey](...entry.args) : {};
  ...
  function withPatchedReferenceContext<T>(referenceContext: Record<string, any> | undefined, cb: () => T): T {
...
function scopeChain<T>(scope: Scope<T> | null) {
  const chain = [];
  while (scope) {
    chain.unshift(scope);
 ...
 * @returns
 */
function resolveDeclarationReferenceScope<T>(target: Declaration<T>, currentScope: Scope<T>) {
